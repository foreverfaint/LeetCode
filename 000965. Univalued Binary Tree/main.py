...
class Solution:
    def foo(self, root, target):
        if not root:
            return True
        return root.val == target and self.foo(root.left, target) and self.foo(root.right, target)

    def isUnivalTree(self, root: Optional[TreeNode]) -> bool:
        if not root:
            return True
        return self.foo(root, root.val)


if __name__ == "__main__":
    sln = Solution()

    tree = TreeNode.from_list([1,1,1,1,1,None,1])
    print(sln.isUnivalTree(tree))

    tree = TreeNode.from_list([2,2,2,5,2])
    print(sln.isUnivalTree(tree))    