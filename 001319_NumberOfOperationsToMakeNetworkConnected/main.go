...
func makeConnected(n int, connections [][]int) int {
	...
	for _, link := range connections {
		...
		for i := 0; i < len(connected); i++ {
			if connected[i] == p1 || connected[i] == p2 {
				connected[i] = new_root
			}
		}
	}

	isolated := make(map[int]bool, 0)
	for _, root := range connected {
		isolated[root] = true
	}
...
func main() {
	...
	fmt.Println(makeConnected(11, [][]int{{1, 4}, {0, 3}, {1, 3}, {3, 7}, {2, 7}, {0, 1}, {2, 4}, {3, 6}, {5, 6}, {6, 7}, {4, 7}, {0, 7}, {5, 7}}))
